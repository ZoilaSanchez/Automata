...
class dibujarAutomata():
    ...
    def dibujarCirculo(self, nombre, final, flecha):
        ...
        return circulo
    #------------------------------------------------------
    def dibujarflechaR(self, letter,flecha,angulo):#De flecha recta
        arrow = turtle.Turtle()
        ...
        return arrow
    #----------------------------------------------------------
    def dibujarflechaC(self, letter,flecha):#De flecha circular
        arrow = turtle.Turtle()
        #Lo movemos mas adelante ya que se dibuja un circulo y las letras
        arrow.penup()
        ...
        return arrow
    #----------------------------------------------------------------------
    def flechaCurveada(self, letter,flecha1, flecha2, inclinacion):#De flecha circular para kleen
        arrow = turtle.Turtle()
        #Lo movemos mas adelante ya que se dibuja un circulo y las letras
        arrow.penup()
    ...
    def crear(self):
        flechas = []
        simbolosJerarquia = ["(", ")"]
        for var in range (0, len(self.cadena)+2):#Agrego una flecha de mas talvez sirva
            var = turtle.Turtle()
        ...
        for var in self.cadena:
            ...
            if(inicio == True and var != "("):
                ...
                if(aux != len(self.cadena)):
                    if(self.cadena[aux+1] == "*"):
                        arreglo2.append(self.cadena[aux+1])#me muevo para saltar parentesis
                        #print("hasta esta posicion llego", self.cadena[aux+1]) 
            ...
            if type(i) == list or type(i) == tuple:
                if(len(arreglo1) == 1): #Si solo es uno con kleene
                    ...
                    if(ban == True):
                        ...
                        bandera = True        
            extra += 1
        flechas[0] = self.dibujarCirculo('q0', bandera, flechas[0])
            ...
            if type(var) == list or type(var) == tuple:
                ...
                if(var[len(var) - 1] == "*"):
                    for i in (var):#Aqui recorremos cada sub arreglo
                        ...
                        if(i != "*"):
                            momentaneo2 = self.dibujarflechaR(i,momentaneo,inclinacion)
                            self.cadena = "q" + str(nombre).strip()#concateno sin espacio
                            print("valor ", i, valor2,"a comparar", len(var)-1)
                            if(valor2 != len(var) - 1):#le restamos el asterisco
                                flechasExtras[valor2]=self.dibujarCirculo(self.cadena, False, momentaneo2)
                            else:
                                flechasExtras[valor2]=self.dibujarCirculo(self.cadena, True, momentaneo2)
                            if(len(var) == 2):#Dibujamos si regresa a si mismo
                                self.dibujarflechaC(var[0],momentaneo2)
                            elif(valor2 + 1 == len(var)):#por el asterisco sumo 1
                                #print("debo llamar a curvear")
                                print("esta letro debo enviar ", var[0])
                                self.flechaCurveada(var[0],flechasExtras[valor2], flechasExtras[1], valor2-1)
                ...
                if(arreglo1[valor-1] != "|"):
                    momentaneo2 = self.dibujarflechaR(var,momentaneo,inclinacion)
                    self.cadena = "q" + str(nombre).strip()#concateno sin espacio
                    if(valor != len(arreglo1)): #Sirve para cuando es a|b  
                        if(arreglo1[valor] == "|"):
                            flechas[valor]=self.dibujarCirculo(self.cadena, True, momentaneo2)
                            ...
                            if(arreglo1[valor][len(arreglo1[valor])-1] == "*"):
                                ...
                                flechas[valor]=self.dibujarCirculo(self.cadena, True, momentaneo2)
                            #flechas[valor]=dibujarCirculo(self.cadena, True, momentaneo2)
                        else:
                            flechas[valor]=self.dibujarCirculo(self.cadena, False, momentaneo2)
